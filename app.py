...
from core.data import load_users, save_users, add_user, edit_user, delete_user, append_user_log, load_user_logs, remove_session, now_iso, session_file
from core.telegram_client import start_client_for_user, confirm_code_for_user, list_dialogs_for_user, send_message_for_user, logout_user_for_user

app = Flask(__name__, static_folder="static", template_folder="templates")
app.secret_key = os.environ.get("SECRET_KEY", "troque_essa_chave")

# Background attack trackers: uid -> {"thread": Thread, "stop_event": Event}
ATTACKS = {}
...
def run_async_in_thread(coro_func, *args, **kwargs):
    """Run an async coroutine in a new event loop and return result (blocking)."""
...
def admin():
    ...
    if request.method == "POST" and request.form.get("action") == "create":
        ...
        try:
            ...
            append_user_log(username, {"action":"created_by_admin","ts":now_iso(), "by": uid})
            msg = "Usuário criado"
            users = load_users()
...
def admin_delete(target):
    uid = session["user"]; users = load_users(); cur = users.get(uid,{})
    if cur.get("role")!="admin":
        return "forbidden",403
    delete_user(target)
    append_user_log(target, {"action":"deleted_by_admin","ts":now_iso(),"by":uid})
    return redirect("/admin")

# ---------- Telegram endpoints (AJAX) ----------
...
def confirm_code():
    ...
    res = run_async_in_thread(confirm_code_for_user, uid, phone, code, phone_hash)
...
# Start a background attack (loop sending) - returns immediately
...
def start_attack():
    ...
    def attack_worker():
        # run Telethon send attempts in loop until success or stop_event is set
        while not stop_event.is_set():
            start_t = time.perf_counter()
            res = run_async_in_thread(send_message_for_user, uid, chat_id, message)
            if res.get("status")=="ok":
                ping = (time.perf_counter() - start_t) * 1000
                append_user_log(uid, {"action":"sent_message","chat_id": chat_id, "ping_ms": round(ping,2), "ts": now_iso()})
                break
            else:
                # Sleep for delay
