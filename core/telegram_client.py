...
def _hash_path_for(phone):
    safe = phone.replace("+", "").replace(" ", "")
    return os.path.join(SESSIONS_DIR, f"{safe}_hash.txt")
...
def api_send_code(phone):
    ...
    try:
        ...
        async def _send():
            ...
            # Salvar hash temporário
            with open(_hash_path_for(phone), "w", encoding="utf-8") as f:
...
def api_confirm_code(phone, code, phone_code_hash=None):
    if not phone:
        return {"status": "error", "error": "phone missing"}
    try:
        # Ler hash salvo se não passado
        if not phone_code_hash:
            hash_file = _hash_path_for(phone)
        ...
        async def _confirm():
            ...
            with open(_session_path_for(phone), "w", encoding="utf-8") as f:
                f.write(ss)
            await client.disconnect()
            return {"status": "ok"}
